...
export async function GET(req) {
  try {
    ...
    // si está vacío, forzamos un punto con /api/feeds/binance
    if (!rows.length) {
    ...
    return withCORS(req, NextResponse.json({ rangeH, count: data.length, data }, { headers:{ 'Cache-Control':'no-store' }}));
  } catch (e) {
